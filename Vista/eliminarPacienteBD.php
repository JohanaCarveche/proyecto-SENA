    ...
    <div class="row">
      <div class="col-sm-4"  style="background-color:#808080">
		...
		<?php
			...
			$documento=$_REQUEST['pacIdentificacion'];
					
			print("<br/>**************<br/>El documento borrado es: $documento</br>**************<br/>");
			

			if( $resultado = $conn->query("DELETE FROM pacientes WHERE pacientes.pacIdentificacion = '".$documento."' ") === TRUE ) {
			echo '<script>alert("Se borró el paciente con éxito")</script>';
					} else {
					echo "Hubo un error ..." ;
					};
			$conn->close();
